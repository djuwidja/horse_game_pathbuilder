...
public class VectorPathCurveTest {
	...
	public void testRoundTrip() throws ConstructorParamException, TrackSectionCurveException {
		...
		VectorPathCurve curve = new VectorPathCurve(pointList);
		testTangentVector(curve, new Point2D.Double(-58.2d, 40d), controlPt, -0.697034446d, 0.717037642d);
		testTangentVector(curve, new Point2D.Double(-58.2d, 38.2d), controlPt, -0.697034446d, 0.717037642d);
		testTangentVector(curve, new Point2D.Double(-58.2d, 31.2d), controlPt, -1, 0d);
		testTangentVector(curve, new Point2D.Double(-205.2d, 187d), controlPt, 0.668964732d, 0.743294146d);
		testTangentVector(curve, new Point2D.Double(479.90951900490893d, -2.6463901925219737d), controlPt, -1d, 0d);
		testTangentVector(curve, new Point2D.Double(486.15951900490893d, -2.6463901925219737d), controlPt, -1d, 0d);
	}
	
	private void testTangentVector(VectorPathCurve curve, Point2D pt, Point2D controlPt, double expectedVecX, double expectedVecY) throws TrackSectionCurveException {
